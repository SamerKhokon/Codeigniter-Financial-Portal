...
class Company_basic_info_ui extends CI_Controller 
{
	...
	{   error_reporting(0);
	    ...
	    $this->load->model("basic_info_data");	
	    $high   = $this->basic_info_data->get_day_high($company_code);
	    $low    = $this->basic_info_data->get_day_low($company_code);
	    $basics = $this->basic_info_data->get_company_basic_info($company_code);  
		...
		$week_52            = $high .'-' . $low;
		$year_end           = $basics_parse[10];
		$market_lot         = $basics_parse[11];
		
		$subsidiaries = $this->basic_info_data->get_subsidiaries($company_code);
		$management = $this->basic_info_data->company_management_info_row($company_code);
		$divident_info = $this->basic_info_data->get_divident_info($company_code);
		...
		$share_info        = $this->basic_info_data->get_share_info($company_code);
		$share_info_parse  = explode("#" , $share_info);
	    $total_share       = $share_info_parse[0];
	    $market_cap        = $share_info_parse[1];
	    $equity            = $share_info_parse[2];	
		
		$Last_Traded_Price = $this->basic_info_data->get_last_trade_price();
		
		$anualized_eps = $this->basic_info_data->get_anualized_eps($company_code);
		if($anualized_eps>0){
		...
		if($anualized_eps>0){
		   $last_audited_eps = $this->basic_info_data->get_last_audited_eps($company_code);
		...
		$paidup_cap = $this->basic_info_data->get_paidup_info($company_code);
		
		$last_info 		         = $this->basic_info_data->get_last_info($company_code);
		...
		$changess 				= $last_info_parse[7];
		$last_time			= $last_info_parse[8];
...
?>
				...
				<div class="col-md-12">
					...
					<table class="table table-responsive">
						<tbody>
							<tr>
								<td>
									<h5 class="pull-left"><?php echo $Last_Traded_Price;?></h5>							
									<?php if($changess<0){?>
								...
								<td>
									<?php if($changess<0){?>
										<small class="red">-<?php echo $changess;?></small><br>
									<?php }else{ ?>
										<small class="blue"><?php echo $changess;?></small><br>
									<?php }?>															
								...
								<td>
									...
									<?php }else{ ?>
										<small class="blue"><?php echo $changess;?>%</small><br>
									<?php }?>
									<small><?php echo $Volume;?></small><br><small><?php echo $Open_Price;?></small></td>
									<td class="center"><br>Last Updated:<?php echo $last_time;?></td>
									<td><small>Pre Close:</small><br><h5><?php echo $Prev_Close_Price;?></h5></td>
									<td><small>High</small><br><small><?php echo $Day_High;?></small><br><small>open:</small></td>
									<td><small>Low</small><br><small><?php echo $Day_Low;?></small><br>
									<small><?php echo $Open_Price;?></small>
								</td>
							</tr>
						</tbody>
					</table>
	...
	public function ui()
	{
	    ...
	    $company_code = $this->input->post("company_code");
	   	$this->load->model("basic_info_data");	
		echo $basics = $this->basic_info_data->get_day_high($company_code);	
